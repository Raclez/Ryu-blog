...
import com.example.demo.xo.mapper.SystemConfigMapper;
import com.example.demo.xo.service.BlogService;

import com.example.demo.xo.service.SystemConfigService;
...
public class SystemConfigServiceImpl extends SuperServiceImpl<SystemConfigMapper, SystemConfig> implements SystemConfigService {
    ...
    public String editSystemConfig(SystemConfigVO systemConfigVO) {
        // 图片必须选择上传到一个区域
        if (EOpenStatus.CLOSE.equals(systemConfigVO.getUploadLocal()) && EOpenStatus.CLOSE.equals(systemConfigVO.getUploadQiNiu()) && EOpenStatus.CLOSE.equals(systemConfigVO.getUploadMinio()) && EOpenStatus.CLOSE.equals(systemConfigVO.getUploadOss())) {
        ...
        // 图片显示优先级为oss优先，必须开启图片上传oss
        if ((EFilePriority.OSS.equals(systemConfigVO.getPicturePriority())
                ||EFilePriority.OSS.equals(systemConfigVO.getContentPicturePriority()))
                && EOpenStatus.CLOSE.equals(systemConfigVO.getUploadOss())) {
            return ResultUtil.errorWithMessage(MessageConf.MUST_BE_OPEN_OSS_UPLOAD);
        }
