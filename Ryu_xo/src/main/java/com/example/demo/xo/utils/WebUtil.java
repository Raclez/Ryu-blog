package com.example.demo.xo.utils;
...
public class WebUtil {
    ...
    public List<String> getPicture(String result) {
        ...
        if (StringUtils.isEmpty(systemConfigJson)) {
            ...
            ossPictureBaseUrl = systemConfig.getOssPictureBaseUrl();
        } else {
            ...
            if (SysConf.SUCCESS.equals(picMap.get(SysConf.CODE))) {
                ...
                if (picData.size() > 0) {
                    for (int i = 0; i < picData.size(); i++) {
                        ...
                        else if (EFilePriority.OSS.equals(picturePriority)){
                            picUrls.add(ossPictureBaseUrl + picData.get(i).get(SysConf.OSS_URL));
                        }
                        else {
    ...
    public List<Map<String, Object>> getPictureMap(String result) {
        ...
        if (SysConf.SUCCESS.equals(picMap.get(SysConf.CODE))) {
                ...
                for (int i = 0; i < picData.size(); i++) {
                    ...
                    if (EFilePriority.QI_NIU.equals(picturePriority)) {
                        ...
                        map.put(SysConf.URL, minioPictureBaseUrl + picData.get(i).get(SysConf.MINIO_URL));
                            ...
                            map.put(SysConf.URL,ossPictureBaseUrl + picData.get(i).get(SysConf.OSS_URL));
                    }
                    else {
            ...
            log.error("获取图片失败，：{}", picMap.get(SysConf.MESSAGE));
        } else {
            log.error("获取图片失败");
        }
        return resultList;
    ...
    public <T> T getData(String result, Class<T> beanType) {
        if (com.example.demo.utils.StringUtils.isEmpty(result)) {
            return null;
        }
