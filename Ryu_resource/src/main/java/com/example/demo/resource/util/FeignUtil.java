...
import com.example.demo.commons.feign.AdminFeignClient;
...
public class FeignUtil {
    ...
    public SystemConfig getSystemConfig() {
        ...
        if (systemConfigMap == null) {
            ...
            String ossAccess = systemConfigMap.get(SysConf.OSS_ACCESS_KEY);
            String ossSecret = systemConfigMap.get(SysConf.OSS_SECRET_KEY);
            String ossBucket = systemConfigMap.get(SysConf.OSS_BUCKET);
            String ossEnpoint = systemConfigMap.get(SysConf.OSS_ENPOINT);
            String uploadoss = systemConfigMap.get(SysConf.UPLOAD_OSS);
            String osspicturebaseurl = systemConfigMap.get(SysConf.OSS_PICTURE_BASE_URL);
            ...
            // 判断oss服务是否存在异常
            if (EOpenStatus.OPEN.equals(uploadoss) && (StringUtils.isEmpty(ossEnpoint) || StringUtils.isEmpty(ossSecret) || StringUtils.isEmpty(ossAccess)
                    || StringUtils.isEmpty(ossBucket) || StringUtils.isEmpty( osspicturebaseurl))) {
            ...
            systemConfig.setOssBucket(ossBucket);
            systemConfig.setOssEndpoin(ossEnpoint);
            systemConfig.setOssAccessKey(ossAccess);
            systemConfig.setOssSecretKey(ossSecret);
            systemConfig.setUploadOss(uploadoss);
            systemConfig.setOssPictureBaseUrl(osspicturebaseurl);
