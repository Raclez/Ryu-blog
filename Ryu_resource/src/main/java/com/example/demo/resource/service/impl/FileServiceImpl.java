...
import com.example.demo.resource.global.MessageConf;
import com.example.demo.resource.global.SQLConf;
import com.example.demo.resource.global.SysConf;
import com.example.demo.resource.mapper.FileMapper;
import com.example.demo.resource.service.*;
import com.example.demo.resource.util.AboutFileUtil;
import com.example.demo.resource.util.FeignUtil;
import com.example.demo.utils.FileUtils;
import com.example.demo.utils.JsonUtils;
import com.example.demo.utils.ResultUtil;
import com.example.demo.utils.StringUtils;
import com.example.demo.base.enums.EFilePriority;
...
public class FileServiceImpl extends SuperServiceImpl<FileMapper, File> implements FileService {
    ...
    public String cropperPicture(List<MultipartFile> multipartFileList) {
        ...
        String ossPictureBaseUrl = systemConfig.getOssPictureBaseUrl();
        ...
        if (SysConf.SUCCESS.equals(picMap.get(SysConf.CODE))) {
            ...
            if (picData.size() > 0) {
                for (int i = 0; i < picData.size(); i++) {
                    ...
                    } else if (EFilePriority.OSS.equals(systemConfig.getContentPicturePriority())) {
                        item.put(SysConf.URL, ossPictureBaseUrl + picData.get(i).get(SysConf.OSS_URL));
                    } else {
            ...
            if (fileList.size() > 0) {
                for (File file : fileList) {
                    if (file != null) {
                        ...
                        // 获取oss地址
                        remap.put(SysConf.OSS_URL, file.getOssUrl());
                        // 后缀名，也就是类型
    ...
    public String batchUploadFile(HttpServletRequest request, List<MultipartFile> filedatas, SystemConfig systemConfig) {
        ...
        String uploadoss = systemConfig.getUploadOss();
        ...
        if (filedatas != null && filedatas.size() > 0) {
            for (MultipartFile filedata : filedatas) {
                ...
                try {
                    ...
                    // 判断是否能够上传oss
                    if (EOpenStatus.OPEN.equals(uploadoss)) {
                        ossUrl = ossService.uploadFile(tempFileData);
                    }
                } catch (Exception e) {
    ...
    public String uploadPictureByUrl(FileVO fileVO) {
        ...
        if (urlList != null && urlList.size() > 0) {
            for (String itemUrl : urlList) {
                ...
                // 上传oss云，判断是否能够上传oss云
                if (EOpenStatus.OPEN.equals(systemConfig.getUploadOss())) {
                    ossUrl = ossService.uploadPictureByUrl(itemUrl, systemConfig);
                }
                File file = new File();
                file.setCreateTime(new Date(System.currentTimeMillis()));
                file.setFileSortUid(fileSort.getUid());
                ...
                if (SysConf.SUCCESS.equals(code)) {
                    ...
                    if (resultList.size() > 0) {
                        ...
                        if (EFilePriority.QI_NIU.equals(systemConfig.getContentPicturePriority())) {
                            ...
                            String ossPictureBaseUrl = systemConfig.getOssPictureBaseUrl();
                            String url = ossPictureBaseUrl + picture.get(SysConf.OSS_URL).toString();
                            map.put(SysConf.URL, url);
                        } else {
                            String localPictureBaseUrl = systemConfig.getLocalPictureBaseUrl();
                            // 设置图片服务根域名
    ...
    public Object ckeditorUploadCopyFile() {
        ...
        if (EFilePriority.QI_NIU.equals(systemConfig.getContentPicturePriority())) {
            ...
            if (StringUtils.isNotEmpty(systemConfig.getLocalPictureBaseUrl()) && StringUtils.isNotEmpty(itemUrl) && itemUrl.indexOf(systemConfig.getLocalPictureBaseUrl()) > -1) {
                ...
                // 表示优先显示oss服务器
                // 判断需要上传的域名和本机图片域名是否一致，如果一致，那么就不需要重新上传，而是直接返回
                if (StringUtils.isNotEmpty(systemConfig.getOssPictureBaseUrl()) && StringUtils.isNotEmpty(itemUrl) && itemUrl.indexOf(systemConfig.getLocalPictureBaseUrl()) > -1) {
                    Map<String, Object> result = new HashMap<>();
                    result.put(SysConf.UPLOADED, 1);
                    result.put(SysConf.FILE_NAME, itemUrl);
                    result.put(SysConf.URL, itemUrl);
            ...
            // 上传七牛云 【判断是否能够上传七牛云】
            if (EOpenStatus.OPEN.equals(systemConfig.getUploadOss())) {
//                qiNiuUrl = ossService.uploadfile( systemConfig);
            }
            ...
            file.setOssUrl(systemConfig.getOssPictureBaseUrl() + ossUrl);
            file.setStatus(EStatus.ENABLE);
            file.setUserUid(userUid);
            file.setAdminUid(adminUid);
            fileService.save(file);
            ...
            } else if (EFilePriority.OSS.equals(qiNiuConfig.get(SysConf.PICTURE_PRIORITY))) {
                result.put(SysConf.URL, systemConfig.getOssPictureBaseUrl() + ossUrl);
            } else {
