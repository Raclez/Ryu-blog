...
import com.example.demo.resource.mapper.NetworkDiskMapper;
import com.example.demo.resource.service.NetworkDiskService;
import com.example.demo.resource.service.StorageService;
import com.example.demo.resource.util.*;
import com.example.demo.resource.vo.NetworkDiskVO;
import com.example.demo.utils.StringUtils;
import com.example.demo.utils.upload.FileUtil;
...
public class NetworkDiskServiceImpl extends SuperServiceImpl<NetworkDiskMapper, NetworkDisk> implements NetworkDiskService {
    ...
    public void deleteFile(NetworkDiskVO networkDiskVO, Map<String, String> qiNiuConfig) {
        ...
        String uploadOss = qiNiuConfig.get(SysConf.UPLOAD_OSS);
        ...
        if (SysConf.ONE == networkDisk.getIsDir()) {
            ...
            if (list.size() > 0) {
                ...
                if (isUpdateSuccess) {
                    ...
                    if (EOpenStatus.OPEN.equals(uploadLocal)) {
                        ...
                        // 批量删除本地图片
                        RyuFileUtil.deleteFileList(fileList);
                    }
                    // 删除七牛云上文件
                    ...
                    if (EOpenStatus.OPEN.equals(uploadOss)) {
                        List<String> fileList = new ArrayList<>();
                        list.forEach(item -> {
                            fileList.add(item.getOssUrl());
                        });
                        String ossBucket = qiNiuConfig.get(SysConf.OSS_BUCKET);

                        OSSUtil.deleteBuchfile(qiNiuConfig,fileList);
                    }
                }
            }
        } else {
            ...
            if (EOpenStatus.OPEN.equals(uploadLocal)) {
                ...
                RyuFileUtil.deleteFile(UPLOAD_PATH + localUrl);
            }
            // 删除七牛云上文件
            if (EOpenStatus.OPEN.equals(uploadQiNiu)) {
